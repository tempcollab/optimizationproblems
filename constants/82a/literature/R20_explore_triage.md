# R20 explore triage — 82a UPPER (Zhang-Zagier essential minimum)

Explorer: math-explorer, round 20. READ-ONLY start-of-round RE-CONFIRMATION triage — no
improvement attempted, no new PDF fetches, no multi-minute computations. On-disk material
only (current.md, R19 + R11 triages, approaches/ + certificate/ + literature/ listings,
role memory).
...
the campaign already sits 1.53e-4 below it and 5.5e-3 below the original Doche bar 0.25444.

This is the **TENTH** consecutive round-start UPPER triage (R11–R19 all DRY; now R20).

## On-disk change check (verified, not on faith)
`ls -t` of certificate/: newest NON-triage artifact is still `verify_dual_loci_decomposition.py`
(R10). Everything after R10 in literature/ is a triage write-up (R11–R19); no new
capacity/identity script, no new digest, no new approach doc, no new certificate since R10.
The standing package is exactly as banked through R10, re-confirmed dry R11–R19. The round-20
dispatch carries NO new idea — it is bit-for-bit the R1 BMQS Thm 4.5 density framing + R6
integer-transfinite-diameter thesis + R7 "minimize r_Q over Z[X]" design principle, all three
already recorded and adversarially closed in R12–R19. Confirmed: nothing on disk changed since
R10/R19.

================================================================================
## Q1 — Any concrete NEW WITD-independent computed artifact (new script -> new identity/number)
## a proof-reviewer would BANK as a new milestone, NOT prose re-assembly of R6–R10?
...
3. **Capacity / transfinite diameter / equilibrium measure of ACT(A)/ACT(B).** REFUTED — do
   ...
   alone, independent of any Q; r_tilde/m_B are weighted log-norms of a FIXED integer Q. Tying
   them IS the open WITD equality inf_Q r_tilde_Q = -log d_inf(ACT(A)), which R7 PROVED FAILS in
   the load-bearing direction (firing integer roots strictly inside the unit disk, |zeta| in
   [0.43,0.79], away from the deepest U^nu wells, so the integer inf sits STRICTLY ABOVE the
   continuous -log d_inf). Also anchor/exponent dependent (|ACT(A)|=0.428 at R4 vs 0.0686 under
   R10 convention) — not one well-defined number. Exactly the trap the dispatch warns against.
...
6. **Consolidate R6–R10 into one theorem.** Prose re-assembly of already-banked facts, no NEW
   identity/script — reviewer rejected this category R9–R19. Banks nothing.

No category yields a concrete, new, one-round, WITD-independent, non-re-run artifact.

================================================================================
## Q2 — Does BMQS Thm 4.5 density (the round-20 user framing), re-examined ONCE MORE, unlock
...
The dispatch framing is the R1 framing already closed R12–R19. The dichotomy is exact
(re-checked against the §4 / §7.3 / Thm 6.5 digests via the R19 + R11 triages):
...
(a) **Strong duality pins the optimum to the saturated cone.** BMQS Thm 6.5: P(g)=inf_mu int g
    ...
    does NOT supply a sub-Doche feasible point.
...
(c) **The density limit has no efficient compass and is high-degree.** Thm 4.5 promises
    ...
    (§1.7): "enormous size of the search space and the lack of an efficient criterion"; "far
    from being practical." A multi-round directed-search gamble, not a one-round construction;
    and if a sequence equidistributes, R7's geometric obstruction (firing roots strictly inside
    the disk vs the locus arc) says integer realizations cannot reach the deepest wells — the
    same wall that saturates the Doche family.

No non-Doche mu_{P,Q} construction is feasible in one round. Definitively re-confirmed (TEN
times: R1 scope, R12–R20).
...
branches reviewer-verified saturated within reach (deg<=16, ~80k candidates, LLL/CVP/well-
rounding, R2+R4 anchors). No unexercised sub-case. The direction is closed.
...
TENTH consecutive dry-well triage. The standing UPPER package is publication-complete:
...
prose re-assembly through R11–R19). No valid builder dispatch -> no build -> no reviewer. END
...
the OSS log-energy certificate — a DIFFERENT dispatch, off-limits in THIS upper dispatch; the
gap to the held upper is now only ~0.00149, so the lower side is where the live frontier is),
...
- Capacity / transfinite diameter / equilibrium measure of ACT(A)/ACT(B): un-theorem'd,
  ...
  Backs nothing. [R11–R19, decisive]
...
- constants/82a/current.md (held upper 0.2538893183 / Bounds / Progress log R6–R10 + R2/R4)
- constants/82a/literature/R19_explore_triage.md (full); R11 (full); R12–R18 referenced
...
- approaches/ + certificate/ + literature/ dir listings (newest non-triage artifact =
  R10 verify_dual_loci_decomposition.py; nothing new since R19 triage)

VERDICT: DRY — no valid builder dispatch (10th consecutive)

...
\newcommand{\repo}{\href{https://github.com/Neehan/zhang-zagier-82a}{\texttt{github.com/Neehan/zhang-zagier-82a}}}
...
\author{Akashnil Dutta\textsuperscript{\dag}}
\address{Akashnil Dutta, Incept Labs}

\author{Adib Hasan\textsuperscript{\dag}}
\address{Adib Hasan, SignalPilot AI}

\author{Tarik Moon}
\address{Tarik Moon, SignalPilot AI}

\thanks{\textsuperscript{\dag} Equal contribution.}
...
has largely been guided by search.  We develop a nonsmooth first-variation calculus for
this selection problem.  Doche's value is recast as the ratio
\[
  \log h(q)=\frac{\int_0^1\max(A_q(s),B_q(s))\,ds}{\max(D_A(q),D_B(q))}
\]
and the directional derivative of its numerator is first proved in an abstract
two-branch model.  Positive-measure branch ties contribute the term
$\int_{\{A=B\}}\max(v_A,v_B)$; the familiar active-arc formula is the non-degenerate
special case.  Combining this with the directional derivative of
$\max(D_A,D_B)$ gives a branch-sensitive one-sided formula in every denominator regime,
including the tie $D_A=D_B$, where the outer maximum produces a genuine kink.  The
result is a regime-correct ``firing'' criterion for deciding whether an absent block
lowers the bound to first order.
...
\]
strictly improving Doche's longstanding $0.25443677$ bound.  This is not the present
record, and the point is methodological rather than record-setting.  Second, applied
diagnostically to the current record of \citet{Gri26}, the criterion fires on each of
its six denominator blocks while natural controls remain inert.  Third, under an
explicit bridge ansatz, it becomes constructive: from the
...
\end{equation}
Both maxima matter.  The numerator has moving switching points and may have genuine
tie sets; the denominator has a possible one-sided kink when $\dA=\dB$.  The first
contribution is to compute the directional derivative of the two-max ratio, and then to
specialize it to the one-block entry tests used in block selection.

\begin{theorem}[Directional first variation; proved in Sections~\ref{sec:firstvar}--\ref{sec:unified}]
\label{thm:main}
Let $A_t=A+t v_A$, $B_t=B+t v_B$ be an integrable one-sided perturbation of the two
branches.  Then
\[
  \Phi'(0;v)=
  \int_{\{A>B\}}v_A\,ds+\int_{\{B>A\}}v_B\,ds
  +\int_{\{A=B\}}\max(v_A,v_B)\,ds .
\]
If $D(t)=\max(D_A(t),D_B(t))$ has one-sided slope $D'(0;v)$ and $D(0)>0$, then
\[
  (\Phi/D)'(0;v)=\frac{1}{D(0)}
  \bigl[\Phi'(0;v)-\log h\cdot D'(0;v)\bigr].
\]
In particular, if a block $Q\in\Z[X]$ is adjoined to one branch of an admissible Doche
family with exponent $q_Q\ge0$, $\log h$ is right-differentiable at $q_Q=0$, and in the
non-degenerate case $\{A=B\}$ is null,
...
dominated-convergence form of the Danskin envelope principle.  When the switching set
has positive measure it is retained explicitly through $\max(v_A,v_B)$; when it is
null, the moving-boundary term vanishes because the two branches agree at the switching
points.  Second, the term $-\log h\cdot s_Q$ is contributed solely by the denominator.
This separation is important at a tie $\dA=\dB$, where the right derivative and left
derivative differ by $\log h\cdot\deg Q/D$.
...
\end{equation}
At an interior optimum inside a fixed finite dictionary this has the expected
reduced-cost reading: active perturbing blocks in the $D_B$-attaining regime sit at
$r_Q=\log h$, and absent blocks must have nonnegative one-sided marginal.  We use only
this first-order interpretation here, not a full KKT or subgradient optimality theorem
for the enlarged infinite block-search problem.
...
\[
  \frac{\log M_z(Q(z(1-z)))+\int_0^1(A-B)^+\,ds}{\deg Q}
...
\ref{sec:unified} prove the abstract envelope derivative and the directional ratio
formula, and Section~\ref{sec:criterion} derives the regime-correct selection
criterion.  Section~\ref{sec:bound} gives the certified improvement over Doche.
Sections~\ref{sec:record} and \ref{sec:generator} diagnose and then generate the
near-cancellation blocks in the current record.  Section~\ref{sec:verify} collects the
certification protocol, and Appendix~\ref{app:data} lists the polynomial data needed to
...
\begin{proposition}[Admissibility check in the present notation]\label{prop:admissibility}
For the Doche families used in Sections~\ref{sec:bound}--\ref{sec:generator}, the
conditions listed in Definition~\ref{def:admissible} are sufficient for Doche's
perturbed-polynomial construction.  The endpoint condition is imposed on the active
perturbing product $W$ appearing in the perturbed denominator factor; factors adjoined
only to the base branch enter the numerator branch and are not subject to this endpoint
restriction.  Thus the base-side blocks $j_3,j_9$ used in Theorem~\ref{thm:upper} do not
have to satisfy $P(0)=P(1)=1$.
\end{proposition}

\begin{proof}
This is the translation of \citet[Lemmas 2--5]{Doc01a} into the two-branch notation
\eqref{eq:AB}--\eqref{eq:Dformula}.  The hypotheses involving the perturbing factor
concern the product $W$ multiplying the distinguished perturbing block in Doche's
perturbed denominator.  In our notation, $W$ is exactly the product of positive-exponent
$B$-side blocks.  Doche's hypotheses for this product are squarefreeness, coprimality
from the base and from the distinguished factor, nonzero degree, and the endpoint
normalization $W(0)=W(1)=1$.  The factors $j_3,j_9$ used in
Theorem~\ref{thm:upper} are adjoined to the $A$-branch, not to $W$; they play the same
role as Doche's original base factors, including $P_1=X$.  Thus the endpoint
normalization is not a condition on base-side factors.  The finite checks recorded in
Table~\ref{tab:verify} verify these algebraic conditions for each active family used
below.
\end{proof}
...
We first isolate the analytic fact from Doche's notation.  Let $(X,\mu)$ be a finite
measure space, let $A,B:X\to\R$ be measurable, finite a.e., with
$\max(A,B)\in L^1(\mu)$, and let $v_A,v_B\in L^1(\mu)$.  Define
\[
  \Phi(t)=\int_X\max(A+t v_A,\,B+t v_B)\,d\mu,\qquad t\ge0.
\]

\begin{theorem}[Envelope derivative with tie sets]\label{thm:abstract-envelope}
The right derivative $\Phi'(0+)$ exists and is
\begin{equation}\label{eq:abstract-envelope}
  \Phi'(0+)=
  \int_{\{A>B\}}v_A\,d\mu+\int_{\{B>A\}}v_B\,d\mu
  +\int_{\{A=B\}}\max(v_A,v_B)\,d\mu .
\end{equation}
\end{theorem}
...
For $t>0$ set
\[
  \psi_t=\frac{\max(A+t v_A,B+t v_B)-\max(A,B)}{t}.
\]
The map $(x,y)\mapsto\max(x,y)$ is $1$-Lipschitz for the $\ell^1$ norm, hence
$|\psi_t|\le |v_A|+|v_B|\in L^1(\mu)$.  Pointwise, if $A>B$ then the first branch
remains dominant for all sufficiently small $t$ and $\psi_t\to v_A$; if $B>A$ then
$\psi_t\to v_B$; and if $A=B$ then
\[
  \psi_t=\max(v_A,v_B)
\]
for every $t>0$.  Dominated convergence gives \eqref{eq:abstract-envelope}.
\end{proof}

Thus no non-degeneracy of the switching set is needed for the derivative itself.  The
hypothesis that the switching set has measure zero is used only when one wants the
simpler active-arc formula with no tie contribution.  We now specialize to Doche's
branches and record the hypotheses used in all one-block applications below.
...
\item[(N)] \emph{(Non-degeneracy.)} $A_0-B\not\equiv0$ on each analytic sub-arc cut out
  ...
  coefficients---so we take (N) as a hypothesis when using the simplified active-arc
  formula.  The full envelope formula \eqref{eq:abstract-envelope} remains valid without
  (N).  Numerically, the kink set $K=\{A_0=B\}$ appears finite in all families used here
  (stable sign-change counts across grid refinements, recorded in
  \texttt{firstvar\_02}), but this numerical audit is only a check on the displayed
  simplification, not an independent algebraic proof of nonidentity.
\end{itemize}
...
\begin{proof}
Apply Theorem~\ref{thm:abstract-envelope} on $X=[0,1]$ with Lebesgue measure,
$v_A(s)=\log|Q(\chi(s))|$ and $v_B(s)=0$.  Hypothesis (L) gives the required
integrability.  The theorem gives
\[
  \Phi^{+\prime}(0)=
  \int_{\{A_0>B\}}\log|Q(\chi)|\,ds
  +\int_{\{A_0=B\}}\max(\log|Q(\chi)|,0)\,ds .
\]
By (N), the switching set is null: on each of the finitely many analytic sub-arcs cut
out by the structural singular set, $A_0-B$ is real-analytic and not identically zero,
so its zeros are isolated, hence finite on compact sub-arcs.  The tie integral therefore
vanishes and \eqref{eq:Phiprime} follows.  Finally $\log h=\Phi/D$ with $D$ constant
near $0$ since the perturbing branch attains $D$.
\end{proof}
...
dominated convergence absorb it.  If $K$ had positive measure, the correct term would
be the explicit tie-set contribution in \eqref{eq:abstract-envelope}.  This is the
Danskin/envelope phenomenon for a parametric integral of a maximum \citep{Ber99}, made
rigorous for the present non-smooth integrand.

%==============================================================================
\section{The unified one-sided marginal}\label{sec:unified}
%==============================================================================

Theorem~\ref{thm:firstvar} assumed the perturbing branch attains $D$. We now remove
that regime assumption.  The natural statement is directional.  Suppose the numerator
branches are perturbed by $A_t=A+t v_A$, $B_t=B+t v_B$, and the denominator arguments
by affine slopes
\[
  D_A(t)=D_A+t\delta_A,\qquad D_B(t)=D_B+t\delta_B .
\]
Let $\Phi=\int\max(A,B)\,d\mu$, let $D=\max(D_A,D_B)>0$, let $\log h=\Phi/D$, and let
$I_D=\{\sigma\in\{A,B\}:D_\sigma=D\}$ be the active denominator set.

\begin{theorem}[Directional derivative of the ratio]\label{thm:ratio-directional}
Under the hypotheses of Theorem~\ref{thm:abstract-envelope}, the one-sided derivative
of
\[
  H(t)=\frac{\int\max(A_t,B_t)\,d\mu}{\max(D_A(t),D_B(t))}
\]
exists and is
\begin{equation}\label{eq:ratio-directional}
  H'(0+)=\frac1D\left[
    \Phi'(0+)-\log h\cdot D'(0+)
  \right],
  \qquad
  D'(0+)=\max_{\sigma\in I_D}\delta_\sigma .
\end{equation}
Equivalently, at a denominator tie,
$\partial D=\operatorname{conv}\{\nabla D_A,\nabla D_B\}$ and the operative feasible
one-sided derivative is the support function
$D'(0+)=\max(\delta_A,\delta_B)$.
\end{theorem}

\begin{proof}
The numerator derivative is Theorem~\ref{thm:abstract-envelope}.  The denominator is
the maximum of two affine functions, so its right derivative is the largest slope among
the affine functions active at $t=0$ \citep[Thm.~23.4]{Roc70}.  Since $D>0$, the
ordinary one-sided quotient rule gives \eqref{eq:ratio-directional}.
\end{proof}

For a single block adjoined at exponent $q_Q=0$, the affine slopes are particularly
simple: only the side to which $Q$ is adjoined has denominator slope $\deg Q$.  Since
$\dA,\dB$ are affine (the $D$-formula \eqref{eq:Dformula}), at $q_Q=0$ exactly one of
three regimes holds: $\dA(0)>\dB$ (I), $\dA(0)<\dB$ (II), or $\dA(0)=\dB$ (III, a tie).
...
\begin{proof}
\emph{The inner derivative is regime-independent.} Apply
Theorem~\ref{thm:abstract-envelope} with $v_X(s)=\log|Q(\chi(s))|$ and with the
direction on the other branch equal to $0$.  By (N), the tie set has measure zero, so
the tie contribution in \eqref{eq:abstract-envelope} vanishes.  Hence
$\Phi^{+\prime}(0)=\int_{\ACT(X)}\log|Q(\chi)|\,ds$ in every denominator regime (with
$\ACT(B)$ for a $B$-side block).
...
The sign of \eqref{eq:unified} is the block-selection test.  It is important not to
normalize blindly: the correct reduced cost depends on which denominator argument is
active at the candidate-free anchor.  Write
\[
  m_A(Q):=\int_{\{A_0>B\}}\log|Q(\chi(s))|\,ds,\qquad
  m_B(Q):=\int_{\{B>A_0\}}\log|Q(\chi(s))|\,ds .
\]
For a block of positive degree, \eqref{eq:unified} gives the following table.

\begin{proposition}[Regime-correct firing tests]\label{prop:regime-table}
Let $Q$ be adjoined to side $X\in\{A,B\}$ at exponent $0$, under \emph{(L)} and
\emph{(N)}.  Then $Q$ fires, i.e. lowers $\log h$ to first order, exactly under the
condition displayed below:
\begin{center}
\begin{tabular}{@{}ccl@{}}
\toprule
side of $Q$ & denominator regime at the anchor & firing condition\\
\midrule
$A$ & $\dA>\dB$ & $m_A(Q)/\deg Q<\log h$\\
$A$ & $\dA<\dB$ & $m_A(Q)<0$\\
$A$ & $\dA=\dB$ & $m_A(Q)/\deg Q<\log h$\\
$B$ & $\dA>\dB$ & $m_B(Q)<0$\\
$B$ & $\dA<\dB$ & $m_B(Q)/\deg Q<\log h$\\
$B$ & $\dA=\dB$ & $m_B(Q)/\deg Q<\log h$\\
\bottomrule
\end{tabular}
\end{center}
At a denominator tie, the entered side becomes denominator-active for $q_Q>0$, so the
right derivative uses the normalized test.
\end{proposition}

\begin{proof}
If $Q$ enters the denominator-attaining side, or if the denominator is tied at the
anchor, then $s_Q=\deg Q$ in \eqref{eq:unified}; the derivative is negative exactly when
$m_X(Q)-\log h\deg Q<0$.  If $Q$ enters the denominator-losing side in a strict regime,
then $s_Q=0$ and the derivative is negative exactly when $m_X(Q)<0$.
\end{proof}

The families constructed in Section~\ref{sec:bound} are in the perturbing-denominator
regime $\dA<\dB$.  Corollary~\ref{cor:criterion} is the corresponding two-line
specialization of Proposition~\ref{prop:regime-table}.
...
\end{equation}
firing iff $\rQ<\log h$.  Thus $\rQ-\log h$ is the reduced cost in this regime.  We do
not need a full KKT theorem in what follows; the only conclusion used is the local
one-sided one: a negative reduced cost gives a feasible first-order descent direction
from the candidate-free anchor.
\end{proof}
...
attaining $D$. Admissibility holds by Proposition~\ref{prop:admissibility} and
Lemma~\ref{lem:doche}: $j_9$ is irreducible
...
$\bigl(\log M_z(Q(z(1-z)))+\int_0^1(A-B)^+\,ds\bigr)/\deg Q$ with
$A=\sum_iq_i\log|P_i(\chi)|$ and $B=\log|Q(\chi)|$, equals Doche's value
\eqref{eq:ratio}: by Jensen
$\int_0^1 B\,ds=\log M_z(Q(z(1-z)))$, so
...
\end{equation}
Here $M_z$ denotes Mahler measure in the $z$-variable; it is not the ordinary Mahler
measure of $Q(X)$ on the unit circle $|X|=1$.
\end{proposition}
...
reduced-cost level $\rQ-\log h\in[+0.0069,+0.0084]$ (spread $0.0015$): not exact
first-order stationarity, but a diagnostic analogue, the multiplicities being fixed at
one rather than freely optimized.
\end{proposition}
...
diagnostic operates on the optimized denominator-first point and the small gap is the
certification margin.

The reading of this section is a \emph{diagnostic}: the criterion fires on $R_0,R_2$
after the fact. The next section makes the corresponding construction explicit: given
the distinguished blocks $Q_1,Q_2$ and the bridge ansatz, $R_0,R_2$ are produced, with
firing transferred by a certified inequality rather than read off a grid.

%==============================================================================
\section{Generating near-cancellation siblings}\label{sec:generator}
%==============================================================================

This section turns the diagnostic of Section~\ref{sec:record} into a conditional
construction.  For the certified bridges considered below, starting from a firing
perturbing block $Q^*$ and a bridge polynomial $g$, the primitive part
$R=\pp(Q^*\mp g)$ is an explicit degree-preserving coprime firing sibling of $Q^*$, and
$R_0,R_2$ are its instances. The construction is generative in the precise sense that
its input is $Q^*$ and the bridge ansatz---not $R_0,R_2$---and its output is the family
they belong to. What stays heuristic is named in the scope remark: the \emph{choice} of
seed and bridge, not the certified firing statement for these bridges.
...
\emph{unconstrained} optimum, but $\deg R_6=27$, so it lies outside the imposed monic
degree-$28$ bridge family and outside the equal-degree comparison used in
Proposition~\ref{prop:restricted-opt} (the margin values for $a\le5$ are reproducible
float midpoints; only their strict ordering is the certified claim). Degree data
verified by \texttt{firstvar\_08} with
...
\begin{equation}\label{eq:transfer-bound}
  \bigl|m_R-m_{Q^*}\bigr|
  \;\le\;\log c\,|\Omega_F|
  +\int_{\Omega_0}\frac{|g/Q^*|}{1-|g/Q^*|}\,ds
...
\[
  \bigl|\log|1\mp u|\bigr|\le-\log(1-|u|)\le\frac{|u|}{1-|u|},
\]
the first inequality because $1-|u|\le|1\mp u|\le 1+|u|$ and
$\log(1+|u|)\le-\log(1-|u|)$, the second the standard bound
$-\log(1-x)\le x/(1-x)$ for $x\in[0,1)$. Integrating gives the bulk term in
\eqref{eq:transfer-bound}.  The cruder bound
$\frac{\theta}{1-\theta}|\Omega_0|$ is available but is not what is used in the
certificate.
...
integral $\int_{\Omega_0}|g/Q^*|/(1-|g/Q^*|)\,ds$, the value
$m_{Q^*}(\Omega_F)$, and the cell-wise well integral
...
shared-normalizer difference identity \eqref{eq:diff-id} apply.  A normalized comparison
between $R_5$ and $R_6$ can still be made, but it belongs to a different comparison
problem.  Proposition~\ref{prop:restricted-opt} therefore proves only the intended
restricted statement: $a=5$ is optimal within the imposed monic degree-$28$ bridge
family.  The $a=6$ case may have a stronger marginal, but it is outside that design
constraint.
\end{remark}
...
As a final application of the criterion, we use the base test of
Corollary~\ref{cor:criterion} as a design objective and ask which additional integer
blocks appear capable of improving the certified family. The blocks of
Section~\ref{sec:bound} enter the base branch, so the relevant quantity is the base marginal
...
repository \repo, in the directory \texttt{certificate/}. The claims fall
...
values but do not enclose them rigorously. The verifier is implemented in Python using
\texttt{numpy}, \texttt{scipy}, \texttt{mpmath}, and \texttt{sympy}; interval endpoints
and directed roundings are handled explicitly by the certificate routines, not assumed
from generic floating-point calls. Table~\ref{tab:verify} keys each statement to its
check, marking the grid diagnostics.
...
\quad active-arc simplification & \texttt{firstvar\_02\_hypotheses} & $K$ appears finite, dominator $L^1$\\
...
Prop.~\ref{prop:admissibility} & \texttt{firstvar\_06\_dictionary} & pairwise coprime, squarefree, endpoints\\

...
with the exponent $5$ the maximal value preserving the monic leading coefficient of
$Q^*$---which also explains why the search stopped there. The one heuristic input is the seed itself; producing it
...
\label{thm:record}
On a fine grid the firing criterion---read in the regime-correct form of
Theorem~\ref{thm:unified} at each candidate-free anchor, where for five of the six blocks
the perturbing branch does not attain $D$ and the test is the active-arc integral
$m_Q=\int_{\{B>A_0\}}\log|Q|<0$---fires, strictly, on every one of the record's six
denominator blocks, while natural controls stay inert; at the record's own optimum the
active blocks cluster at a common reduced-cost level. The marginals are grid evaluations,
stated as numerical diagnostics.
\end{proposition}
...
transfers the firing from $Q^*$ to $R$: $R$ fires on the anchor (in the base regime of
$F$, $m_R<0$). The Grinsztajn near-cancellation factors $R_0,R_2$ are exactly the
instances $Q^*\in\{Q_1,Q_2\}$, and $a=5$ in $g$ is the maximal exponent preserving the
monic leading coefficient of $Q^*$ (Proposition~\ref{prop:degfloor})---which is why the
search stopped there (Remark~\ref{rem:a6}).
\end{theorem}
...
\toprule
block & degree & $\dB$ & regime & $m_Q$ (cand.-free) & $\rQ-\log h$ at opt. & fires?\\
\midrule
...
piece. The content term contributes $-\log c$ uniformly, hence $\log c\,|\Omega_F|$ in
...
integral to the true marginal shift ($\approx10^{-2}$, cf.\ \eqref{eq:transfer-id}).

...
overhead). Loads diff/max from d150_diffmax.json for the chosen T, persists
beat_d150.json. The chosen T is read from argv[1] (default 282)."""
import os, sys, json, time, math
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(HERE, "..", "engine"))
from digit_dp import count_opset

A = [0, 2, 3, 4, 5, 6, 7, 8, 9, 10]; B = 21; d = 150
T = int(sys.argv[1]) if len(sys.argv) > 1 else 282
dm = json.load(open(os.path.join(HERE, "d150_diffmax.json")))
cell = dm["cells"][str(T)]
...
out = {"cell": f"d150_T{T}", "A": A, "B": B, "d": d, "T": T, "density": T / d,
       "Nplus": str(Np), "Nminus": str(Nm), "maxU": str(M),
       "value_float": v, "value_record_float": 1.1740744476935212,
       "elapsed_s": el, "done": True}
json.dump(out, open(os.path.join(HERE, "beat_d150.json"), "w"), indent=2)
with open(os.path.join(HERE, "run_d150_engine.log"), "w") as f:

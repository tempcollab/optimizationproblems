...
```
Result: `Build completed successfully (2412 jobs).` (exit 0). Warnings only — the `sorry`s on
...
```
Both **sorry-free and axiom-clean** (standard Mathlib trio only — NO `sorryAx`). These are the
genuinely-discharged sub-steps of H_GEN_τ. The remaining theorems
(`generic_cover_le_13`, `generic_regime_thirteen`) still carry `sorryAx` — honest open holes.
...
- **H_GEN_EDGES** — the 12 edges in Lean (needs a multi-D interval-covering primitive).

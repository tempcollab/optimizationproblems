...
against the trusted engine count_opset on a small cell before being trusted at d=170.

Usage:
  python sumset_progress.py validate     # cross-check vs engine on small cells
  python sumset_progress.py run <T>      # run d=170 sumset, persist beat_d170.json
"""
...
def run(T):
    ...
    out = {"cell": f"d170_T{T}", "A": A_FAMILY, "B": B, "d": D, "T": T, "density": T / D,

"""Compute the CHEAP pieces (diff |U-U| and max(U)) for d=150 over a small T
neighborhood {281,282,283} and persist to d150_diffmax.json. The heavy sumset
runs in a separate call for the single chosen T.

diff DP is ~135s per T (collapsed minSa-state); max(U) is greedy (instant)."""
...
for T in Ts:
    ...
    json.dump(out, open(os.path.join(HERE, "d150_diffmax.json"), "w"), indent=2)

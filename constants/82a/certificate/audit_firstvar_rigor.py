...
Two audits the written proof needs as backing (the proof body itself is by hand):

(K) KINK-SET FINITENESS / STABILITY.  K = {s in [0,1] : A_0(s) = B(s)} is the boundary
    of the active arc {A_0 > B}.  A_0 - B is real-analytic OFF the finite contour-root
    set of its blocks and is NOT identically 0, so K is finite.  We AUDIT that the number
    of sign-changes of A_0 - B is STABLE across N (a finite, N-independent count = the
    numerical fingerprint of a finite K), on the held R4 family.

(L) CONTOUR-ROOT-FREE (H1') for the CANDIDATE Q.  For DCT domination we need
    log|Q(chi(s))| in L^1, i.e. the candidate Q has no root ON the contour {chi(s)}; audit
    min_s |Q(chi(s))| > 0 for the fired candidate blocks j3, j9 (the lemma's test blocks).
    (A_0, B themselves DO have integrable -inf log-singularities -- H1'' -- where a base
...
def main():
    ...
    print("=" * 88)
    print("\n(K) KINK-SET finiteness: sign-changes of A_0 - B, stability across N")
    ...
    print("\n(L) CONTOUR-ROOT-FREE (H1') for candidate blocks: min_s |Q(chi(s))| > 0")
    N = 8_000_000
    ...
    print("\n  (H1'') base/perturber blocks P1=X etc. DO vanish on the contour "
          "(chi(0)=0),")

"""
Re-runnable STRICT-BEAT certificate for C_3a (Round 25, d=160 cell).
...
Construction (the record drop-1 alphabet, pushed to larger d):
    ...
    NEW cell:  d=160, T=300 (density 1.8750)  ->  value (float, display only) printed below.

ALL load-bearing arithmetic is EXACT big-int -- no float decides anything:
  Step 1  load the persisted d=160 integers (re-derivable from scratch with --recompute:
          diff/max ~230s + sumset ~1050s via engine/digit_dp.count_opset).
  ...
  Step 5  EXACT rational re-assertion that c strictly exceeds ALL prior bars:
            c > 239/203 (R24 HELD) > 1177/1000 > 2353/2000 > 5877/5000 > the record value.

Run:  python3 verify_beat.py              (loads persisted d=160 integers; ~80s)
      python3 verify_beat.py --recompute  (also recomputes the d=160 cell from scratch)
...
HELD   = Fraction(239, 203)    # R24 held (verified, Lean), 1.1773399…
HELD3  = Fraction(1177, 1000)  # R23 held, 1.1770
HELD2  = Fraction(2353, 2000)  # R22 held, 1.1765
HELD0  = Fraction(5877, 5000)  # R18/R19 held, 1.1754

# The chosen wedge is read from beat_d160.json (written by the build step) so this
# script stays in sync with the Lean cert. Falls back to the recommended 27/152.
persisted = json.load(open(os.path.join(HERE, "beat_d160.json")))
P_w = persisted.get("wedge_P", 27)
Q_w = persisted.get("wedge_Q", 152)
...
if RECOMPUTE:
    print("    --recompute: recomputing d=160 integers from scratch (~1300s) ...")
    Nm2 = count_opset(A, d_new, T_new, '-')
    M2  = max_U(A, B, d_new, T_new)
    Np2 = count_opset(A, d_new, T_new, '+')
    assert Np2 == Np and Nm2 == Nm and M2 == M, "d160 recompute disagrees with persisted"
    print("    d=160 from-scratch recompute matches persisted integers: OK")
else:
    print("    d=160 integers loaded from persisted beat_d160.json "
...
all_ok = (new_ge and rec_lt and beats_held and beats_held3
          and beats_held2 and beats_held0 and beats_rec)
print()
if all_ok:
    print(f"CERTIFICATE OK: C_3a >= {WEDGE} = {float(WEDGE):.12f}")
    print(f"  > held 239/203 = 1.1773399          (strict beat of OUR R24 held bound)")
    print(f"  > 1177/1000 = 1.1770                (strict beat of the R23 held bound)")

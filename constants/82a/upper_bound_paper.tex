...
on, and accounts for, the blocks of the \citet{Gri26} construction---it does not
\emph{generate} the near-cancellation factors $R_0,R_2$, which are that author's
construction, but diagnoses after the fact that they fire, that natural controls do not,
and that the record sits near a common reduced-cost level (a diagnostic analogue of KKT
stationarity, the multiplicities being fixed). The marginals there are grid evaluations,
not interval certificates; we state them as numerical diagnostics. The improvement of
...
\citet{Gri26}---but diagnoses, after the fact, that they fire, that natural controls do
not, and that the record sits near a common reduced-cost level. The two constructions
optimize the same Doche functional; the record is consistent with, and sharply
diagnosed by, the first-variation criterion. We do not claim the search optimized the
marginal rule, only that its output is exactly what the rule selects.

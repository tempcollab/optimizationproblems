...
held: lower 0.24874 (reviewer-verified, R1; RIGOROUS interval branch-and-bound
      ...
      upper 0.2542657872 (REVIEWER-VERIFIED, R9 — see Progress log R9). RECORD BREAK
      tightening the reviewer-verified R7 held 0.2543185491 by 5.276e-5 (still <
      Doc01b 0.25443677). A SECOND free-exponent perturbing block Q4=Qb (deg 24)
      added to the R7 family: base {P1,P2,P4,P6,P8}, FIXED distinguished block Q1*Q2
      (deg 56, exponent 1), free block Q3=Qa (deg 24, exponent qB) and NEW free block
      Q4=Qb (deg 24, exponent qC). q=(13.5067,9.9134,2.7258,1.7086,0.7364), qB=0.1092,
      qC=0.2437; B=log|Q1Q2|+qB*log|Q3|+qC*log|Q4|, D=max(53.8833, 56+qB*24+qC*24)=
      64.4696 (Doc01a §4 D-formula with ell=2). Frontier fully resolved (665192 leaves,
      0 unresolved, 6 rounds), int_0^2pi G dt <= 102.9965722302, int_0^1 G ds <=
      16.3924135920, 16.3924135920/64.4696 = 0.2542657872. Reproduce: python3
      constants/82a/certificate/verify_upper_q4.py certify 13.5067 9.9134 2.7258 1.7086
      0.7364 0.1092 0.2437 200000 14 1e-10 (~3.5 min; prints admissibility of
      W=Q1*Q2*Q3*Q4 incl. gcd(Qa,Qb)=1, selftest_q4 0/200 on the CHANGED integrand,
      then CERTIFIED 0.2542657872). Anchor (qC=0) recovers the R7 (q3) family value to
      >=10 digits (D=59.096); tamper (bogus 0.25425 -> BEATS=False); reviewer's own
      mpmath prec-200 mp.quad cross-check on 30 cells (x2 caps) worst +2.96e-16 >= 0.

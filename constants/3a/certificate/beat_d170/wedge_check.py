"""Exact big-int / Fraction wedge confirmation for the d=170 beat.

Confirms, with NO floats on the load-bearing comparison:
  (a) d=170 cell PASSES   Nminus^Q >= Nplus^Q * (2*maxU+1)^P   (c-1 = P/Q)
  (b) d=80 RECORD cell FAILS the same wedge  (=> c strictly above true record)
  (c) chosen c > held 179/152 exactly via Fraction
...
# --- load beat cell (d=170) ---
beat = json.load(open(os.path.join(HERE, "beat_d170.json")))
    ...
    while True:
        ...
        if c.denominator != Q:   # not reduced -> skip (cheaper Q handles it)
            ...
            if float(c) > 1.18:
                break
            continue
        if float(c) > 1.18:       # above measured value, stop
            break
...
for c in candidates:
    ...
    print(f"  (a) d=170 cell passes wedge : {a}")
    print(f"  (b) d=80 record cell passes : {b}  (must be False => record FAILS => strict over record)")
    print(f"  (c) c > held 179/152        : {over}   margin = {float(c-held):.3e}")

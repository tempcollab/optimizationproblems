...
explicit finite search---that the cheap improving blocks have been exhausted, locating
where Doche's construction reaches the floor of its reachable gains. Code at \repo.
\end{abstract}
...
\emph{(H1)--(H3)} of Section~\ref{sec:firstvar} (in particular $A_0-B\not\equiv0$).
...
Finally, run as a design objective the criterion locates where the construction's
reachable gains run out. The base marginal $\widetilde r_Q$ is additive over
factorizations in $\Z[X]$, and an explicit finite search finds that the cheap firing
...
claim only that the \emph{reachable} improvements are exhausted, and explain the
mechanism.
...
We use the base criterion of Corollary~\ref{cor:criterion} as a design objective and
ask which integer blocks improve the bound. The blocks of
Section~\ref{sec:bound} enter the base branch, so the relevant quantity is the base
marginal $\widetilde r_Q=\int_{\{A_0>B\}}\log|Q(\chi)|\,ds$, with $Q$ firing iff
...
\emph{certifiably} improving blocks of this construction have been exhausted---not that

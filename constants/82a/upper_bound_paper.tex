...
obtained by an independent denominator-first search, the criterion fires on
\emph{every} block of that construction---including two new near-cancellation factors
our analysis had not seen---while staying inert on natural controls, and the record's
active blocks cluster at a common reduced-cost level. The two constructions optimize the
same Doche functional; the record is consistent with, and sharply diagnosed by, the
first-variation criterion. We also give computational evidence---an additivity property
of the marginal with an explicit finite search---that within a specified finite
dictionary the cheaply improving blocks have already been absorbed. Code at \repo.
\end{abstract}
...
construction should carry, the record constructions among them. We test it against a
construction it did not produce.
...
denominator-first search, optimizes the same Doche functional \eqref{eq:ratio} written
in complementary coordinates. Its denominator carries six perturbing blocks, among them
two new near-cancellation factors $R_0,R_2$ outside the dictionary studied here. On a
fine grid the perturbing criterion fires on \emph{every} one at the candidate-free
anchor ($\rQ<\log h$, strictly), while natural controls stay inert; and at the record's
own optimum the active blocks cluster at a common reduced-cost level. The record is thus
consistent with, and sharply diagnosed by, the criterion (Proposition~\ref{thm:record-fires}).
\end{proposition}

The diagnosis is strengthened to a \emph{construction}. From a distinguished block
$Q^*$ and an admissible bridge polynomial $g$, the criterion generates a firing sibling.

\begin{theorem}[Sibling generator; Section~\ref{sec:generator}]\label{thm:gen-intro}
Let $Q^*\in\{Q_1,Q_2\}$ be a firing degree-$28$ block of Doche's distinguished block on
...
factors $R_0,R_2$ are the instances of this construction, and the exponent $a=5$ in $g$
is the maximal value preserving the degree (Proposition~\ref{prop:degfloor}). The seed
$Q^*$ and the bridge ansatz are inputs; their optimality is not claimed.
\end{theorem}
...
Theorem~\ref{thm:gen-intro} (Section~\ref{sec:generator}) strengthens this from
diagnosis to \emph{construction}, but only \emph{from a seed}: given the distinguished
block $Q^*$ and the bridge ansatz it generates the firing sibling family containing
$R_0,R_2$, with the firing transferred by a directed-rounded interval certificate
(Lemma~\ref{lem:transfer}) and the maximal exponent $a=5$ forced by an exact degree
count (Proposition~\ref{prop:degfloor}). We do \emph{not} claim the criterion alone, with
no seed, predicts $R_0,R_2$, nor that the seed or bridge is optimal among generators
(Remark~\ref{rem:gen-scope}); the choice of seed and bridge stays heuristic. The improvement of
...
$\log|R_{a+1}|-\log|R_a|$; it is not one-signed on $\Omega_F$ (for $a=4$ about $79\%$ of
the mass is negative but the integrand reaches $+2.78$ on the positive part), and

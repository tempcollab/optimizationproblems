...
denominator-first search to a certified $0.2536331090$, distributed by \citet{Gri26} as
an independent reproducible code certificate (we cite the archived repository, not a
manuscript). Thus
\[
...
the seed block $Q^*$ and the sibling $R$ removed, with perturbing active set
...
from the family it is scored against. Both $Q^*$ and $R$ are then scored on this single
$\Omega_F$, the marginal $\rQ$ of Definition~\ref{def:rq} is unambiguous, and
``$Q^*$ fires'' reads $\rb{Q^*}(\Omega_F)<\log h$ as a genuine entry test.
...
guaranteed to fix the degree and leading coefficient \emph{for either sign}. The
behaviour at the boundary $a=6$ is sign-dependent, and this is where the two record
factors differ: for the subtraction $\pp(Q_1-g_6)$ with tail $P_8$ the degree-$28$
leading terms cancel ($1-1=0$) and the degree drops to $27$, so $a=5$ is sharply maximal;
for the addition $\pp(Q_2+g_6)$ with tail $P_7$ they add ($1+1=2$) and $a=6$ still has
degree $28$ but leading coefficient $2$, so there $a=5$ is the maximal
\emph{monic}-preserving exponent. Either way $a=5$ is the maximal exponent preserving both the degree and the
leading coefficient of $Q^*$, and $R_0,R_2$ are the instances $a=5$.
\end{proposition}
...
denominator, so $\rb{Q^*}(\Omega_F)$ and $\rb{R}(\Omega_F)$ are genuine entry tests
(Remark~\ref{rem:anchor}), not scores against a family already containing $R$. Second,
the upper bound on $\rb{Q^*}=\int_{\Omega_F}\log|Q^*|\,ds$ is interval-sound on cells
...
certified constants,
\[
  \rb{Q_1}(\Omega_F)\le-0.04216,\quad \mathrm{RHS}\le0.00379,\quad
  \rb{Q_1}+\mathrm{RHS}\le-0.03837<\log h=0.25363
\]
for $R_0$, and $\rb{Q_2}(\Omega_F)\le-0.03536$, $\mathrm{RHS}\le0.00444$,
$\rb{Q_2}+\mathrm{RHS}\le-0.03092<\log h$ for $R_2$. The firing margins
...
\[
  D_1^{\mathrm{hi}}=-2.25\times10^{-3},\;
  D_2^{\mathrm{hi}}=-1.97\times10^{-3},\;
  D_3^{\mathrm{hi}}=-1.71\times10^{-3},\;
  D_4^{\mathrm{hi}}=-4.33\times10^{-4},
\]
all strictly negative, with $\inf_{\Omega_F}|R_a|>0$ certified for $a=1,\dots,5$ and no
unresolved cells. (The true differences are $-6.86,-5.25,-3.56,-1.84$ in units of
...
$R_a=\pp(Q_1-g_a)$ the exponent $a=6$ gives a sibling that fires \emph{harder} than
$R_0$ ($\rb{R_6}\approx-0.0523<\rb{R_5}\approx-0.0498$ on $\Omega_F$), so ``$a=5$ is
...
recorded with the submission's archived artifact (DOI assigned at acceptance); the

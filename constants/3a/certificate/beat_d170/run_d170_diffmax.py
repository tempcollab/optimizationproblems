"""Compute the CHEAP pieces (diff |U-U| and max(U)) for d=170, T=319 and persist
to d170_diffmax.json. The heavy sumset runs in a separate call.

diff DP uses the collapsed (minSa,Sc) state (deterministic min-transition), ~250s
at d=170; max(U) is greedy (instant)."""
...
for T in Ts:
    ...
    json.dump(out, open(os.path.join(HERE, "d170_diffmax.json"), "w"), indent=2)

...
UPDATE (this branch): a real bug in the per-cell separation was fixed -- |chi-alpha|^2
is now enclosed by a proper interval-SQUARE (_isq), not the generic interval-multiply,
which returned spuriously NEGATIVE lower bounds for straddling intervals and floored
g_lo at LOG_FLOOR.  The per-alpha-cell s-loop is also restricted to could_in s-cells
(certainly_out contribute 0) for ~3.2x less work, and the box-B cover now prints a
per-depth progress line.  These are sound (verified U_lo <= true U) and necessary, but
they do NOT by themselves make box B converge: the dominant blocker is the worst-point
log bound over contour-CROSSING s-cells (over-counts negative mass; alpha-bisection
cannot resolve a cell the contour passes through).  The pending fix is a sound analytic
bound on the log-dip integral over such s-cells; box B remains NOT YET PASSING.

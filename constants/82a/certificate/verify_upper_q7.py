...
Family (Doche general-l, multi-free-block perturber):
  ...
  Q3=Qa free (qB>=0); Q4=Qb free (qC>=0); Q5=j13 deg12 free (qE>=0);
  Q6=j15 deg16 free (qF>=0); Q7 = NEW low-deg block free (qG>=0).

  A(t) = sum_i q_i * (1/2) log|P_i(w(t))|^2                       (prod-P^q branch)
  B(t) = (1/2)log|Q1|^2 + (1/2)log|Q2|^2
          + qB*(1/2)log|Q3|^2 + qC*(1/2)log|Q4|^2
          + qE*(1/2)log|Q5|^2 + qF*(1/2)log|Q6|^2
          + qG*(1/2)log|Q7|^2                                     (Q-branch)
  G(t) = max(A(t), B(t))
...
the ZZ spectrum for any admissible (q, all exponents >= 0) (no optimality needed),
hence C_82 <= log h.  Admissibility of the active dictionary is verified by
admissibility_check_q7() -- for the certified Angle (qB=qC=0) the active dict is
W = Q1*Q2*Q5*Q6*Q7 (all factors non-constant & pairwise coprime, W(0)=W(1)=1, Q7
squarefree, gcd(Q7, each kept factor)=1 incl. the LOAD-BEARING inter-block
gcd(Q7,Q5)=gcd(Q7,Q6)=1).

The Q7 block is selectable via env var Q7_CAND in {j3,j5,j6,j7,j9} (default j3),
...
# Held R11 blocks (identical to q6).
...
# ---------------------------------------------------------------------------
# Q7 = a LOW-DEGREE Flammang Table-1 block (DESCENDING in X, harness convention,
# like Q3/Q4/Q5/Q6).  Selectable via env var Q7_CAND.  These coeff lists are
# transcribed verbatim from flammang_table1.py _TABLE_DESCENDING (the printed
# Table-1 form is already descending in w=X=z(1-z)).
# ---------------------------------------------------------------------------
_Q7_CANDS = {
    "j3": [1, 1, -2, 1],                                   # w^3+w^2-2w+1, deg 3
    "j5": [1, -2, 4, -3, 1],                               # deg 4
...
def _Dval(q, qB, qC, qE, qF, qG):
    return max(float(np.dot(q, DEGP)),
               float(DEGQ12 + qB * DEG_Q3 + qC * DEG_Q4
                     + qE * DEG_Q5 + qF * DEG_Q6 + qG * DEG_Q7))


# ---------------------------------------------------------------------------
def cell_AB_q7(a, b, q, qB, qC, qE, qF, qG):
    m = 0.5 * (a + b)
    r = 0.5 * (b - a)
    ...
    # ---- B branch: Q1 + Q2 (fixed) + qB*Q3 + qC*Q4 + qE*Q5 + qF*Q6 + qG*Q7 ----
    ...
    Bfactors = [("Q1", vu.ASC["Q1"], 1.0),
                ...
                ("Q5", ASC_Q5,       float(qE)),
                ("Q6", ASC_Q6,       float(qF)),
                ("Q7", ASC_Q7,       float(qG))]   # NEW low-degree free block
...
def certify_maxAB_q7(q, qB, qC, qE, qF, qG, label, target, M0=200000,
                     max_refine=14, rem_cap=1e-10, verbose=True):
    """Guaranteed UPPER bound on log h(q,qB,qC,qE,qF,qG) via max(A,B) enclosure."""
    ...
    while True:
        cell_hi, refine = cell_int_maxAB_q7(a, b, q, qB, qC, qE, qF, qG, rem_cap)
        keep = ~refine
        total_resolved = na(
...
# ---------------------------------------------------------------------------
def float_value_q7(q, qB, qC, qE, qF, qG, N=8_000_000):
    """Float midpoint Riemann sum of log h(q,qB,qC,qE,qF,qG) (CONJECTURE / anchor)."""
    s = (np.arange(N) + 0.5) / N
    z = np.exp(2j * np.pi * s)
    chi = z * (1 - z)
...
def admissibility_check_q7(active_only=True):
    """Admissibility of the active perturbing dictionary W=Q1*Q2*Q5*Q6*Q7
    (the CERTIFIED Angle, qB=qC=0).  Verifies Doche condition (4): deg Q7>0,
    Q7(0)=Q7(1)=1, Q7 squarefree, Q7 coprime to every kept factor incl. the
    LOAD-BEARING inter-block gcd(Q7,Q5)=gcd(Q7,Q6)=1, and the full pairwise grid."""
    import sympy as sp
    X = sp.symbols('X')
    ...
    print(f"  [Q7 = Flammang {Q7_CAND}]")
    print(f"  deg_X Q7 = {DEG_Q7} (>0: {DEG_Q7 > 0})")
    print(f"  Q7(0) = {Q7s.subs(X, 0)}  Q7(1) = {Q7s.subs(X, 1)}")
    q7_norm = (int(Q7s.subs(X, 0)) == 1 and int(Q7s.subs(X, 1)) == 1)
    sq7 = (sp.gcd(Q7s, sp.diff(Q7s, X)) == 1)
    print(f"  Q7(0)=Q7(1)=1: {q7_norm}   Q7 squarefree: {sq7}")

    W = sp.expand(Q1s * Q2s * Q5s * Q6s * Q7s)
    degW = sp.degree(W, X)
    W0, W1 = int(W.subs(X, 0)), int(W.subs(X, 1))
    print(f"  [certified family, qB=qC=0]  deg_X W=Q1*Q2*Q5*Q6*Q7 = {degW}  "
          f"W(0)={W0}  W(1)={W1}")
    ...
    for nm, Pc in [("P1", vu.P1), ("P2", vu.P2), ("P4", vu.P4),
        ...
        print(f"  gcd(Q7,{nm})=1: {g}")
    g71 = sp.gcd(Q7s, Q1s) == 1
    g72 = sp.gcd(Q7s, Q2s) == 1
    print(f"  gcd(Q7,Q1)=1: {g71}   gcd(Q7,Q2)=1: {g72}")
    kept_ok = kept_ok and g71 and g72

    g75 = sp.gcd(Q7s, Q5s) == 1
    g76 = sp.gcd(Q7s, Q6s) == 1
    print(f"  *** LOAD-BEARING inter-block *** gcd(Q7,Q5)=1: {g75}   "
          f"gcd(Q7,Q6)=1: {g76}")
    kept_ok = kept_ok and g75 and g76

    g73 = sp.gcd(Q7s, Q3s) == 1
    g74 = sp.gcd(Q7s, Q4s) == 1
    print(f"  *** ell-extended inter-block *** gcd(Q7,Qa)=1: {g73}   "
          f"gcd(Q7,Qb)=1: {g74}")

    certified_ok = ((DEG_Q7 > 0) and q7_norm and sq7 and nox and kept_ok)
    full_ok = certified_ok and g73 and g74
    print(f"  CERTIFIED-FAMILY admissibility (W=Q1*Q2*Q5*Q6*Q7, Doche Lemma 5): "
          f"{certified_ok}")
    print(f"  FULL admissibility (incl. Qa,Qb inter-block): {full_ok}")
    return certified_ok if active_only else full_ok


def selftest_q7(q, qB, qC, qE, qF, qG, ntest=200, seed=11):
    """Soundness on the CHANGED integrand: cell_int_maxAB_q7 must dominate the true
    int_cell max(A,B) dt (mpmath, high precision).  B now has the extra qG*log|Q7|."""
    import mpmath as mp
    import random
    mp.mp.prec = 160
    ...
    for cap, tag in [(0.0, "flat"), (1e-9, "midpt")]:
        cell_hi, refine = cell_int_maxAB_q7(A, B, np.asarray(q, float),
        ...
        print(f"  selftest_q7[{tag}]: {viol}/{ntest} violations  (MUST be 0)  "
...
if __name__ == "__main__":
    ...
    # R11 (q6) optimum (the qG=0 anchor point): the held family h=Q1*Q2*Q5^qE*Q6^qF.
    ...
    if mode == "anchor":
        ...
        print(f"  float_value_q7(R11 q, qB=qC=0, qE, qF, qG=0) = {v7:.12f}")
        ...
        print(f"  D_q7(qG=0) = {D7}   D_q6 = {D6}   match: {D7 == D6}")
        aa = np.array([1.0, 2.0, 3.0]); bb = aa + 1e-4
        c7, _ = cell_int_maxAB_q7(aa, bb, np.array(R11_Q), 0.0, 0.0, R11_QE,
                                  R11_QF, 0.0, 1e-10)
        ...
        print(f"[selftest_q7] Q7={Q7_CAND} q={q} qB={qB} qC={qC} qE={qE} qF={qF} qG={qG}")
        ok = selftest_q7(q, qB, qC, qE, qF, qG)
        ...
        print(f"=== TAMPER Q7={Q7_CAND} q={q.tolist()} qB={qB} qC={qC} qE={qE} "
              f"qF={qF} qG={qG} bogus={bogus} ===")
        logh_hi, nbad, _, _, _, _, _ = certify_maxAB_q7(
        ...
        print(f"=== certify_q7 Q7={Q7_CAND} q={q.tolist()} qB={qB} qC={qC} "
              f"qE={qE} qF={qF} qG={qG} ===")
        print(f"    M0={M0} max_refine={max_refine} rem_cap={rem_cap}")
        print("\n[admissibility] active perturbing dictionary (Doche Lemma 5)")
        adm = admissibility_check_q7()
        if not adm:
            print("    ABORT: admissibility failed.")
            sys.exit(1)
        print("\n[float] log h(q,qB,qC,qE,qF,qG) ~ (CONJECTURE)")
        vq = float_value_q7(q.tolist(), qB, qC, qE, qF, qG, N=8_000_000)
        print(f"    float log h = {vq:.10f}  h = {math.exp(vq):.10f}")
        print("\n[selftest_q7] soundness of per-cell bound on THIS integrand")
        ok = selftest_q7(q.tolist(), qB, qC, qE, qF, qG)
        print(f"    selftest passed: {ok}")
        if not ok:
            print("    ABORT: selftest failed, certificate not trustworthy.")
            sys.exit(1)
        print("\n[certify] rigorous max(A,B) quadrature enclosure")
        logh_hi, nbad, elapsed, n_leaf, tot, ints, D = certify_maxAB_q7(
            q, qB, qC, qE, qF, qG, "q7", HELD_CERT, M0=M0, max_refine=max_refine,

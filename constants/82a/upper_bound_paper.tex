...
derivative two consequences follow. First, a firing criterion: a block lowers the
bound to first order according to a sign condition on a single arc integral, with the
condition taking one form for a base block (no value term) and another for a
perturbing block (the complementary-slackness condition $\rQ=\log h$ of the
associated measure linear program). Second, run as a design objective the criterion
selects two further admissible blocks, and certifying the resulting family yields the
improved record
\[
  \essmin\le 0.2538893183,
\]
beating $0.25443677$ \citep{Doc01b} by an outward-rounded interval enclosure. We also
give computational evidence---an additivity property of the marginal together with an
explicit finite search---that within a specified finite dictionary the cheaply
improving blocks have already been absorbed, indicating where the easily reachable
gains of the construction run out. Code at \repo.
\end{abstract}
...
that does not attain $D$; it is the test that selects the blocks of the improved bound
(Section~\ref{sec:bound}). Run as a design objective over integer blocks, either test
bounds the construction.

Carried out, the criterion selects two further admissible blocks and gives the bound.
...
The improvement of Theorem~\ref{thm:upper} is genuine but small; its role is to
confirm that the analysis selects the blocks it predicts. The saturation discussion of
...
The improvement is modest in absolute terms but a definite fraction of the open
interval, as Table~\ref{tab:gap} records; its role is to certify that the criterion
selects the blocks it predicts, not to settle the constant.
...
\begin{tabular}{@{}lll@{}}
\toprule
 & upper bound & gap above $0.2487458$ \citep{F18}\\
\midrule
previous record \citep{Doc01b} & $0.25443677$ & $5.690\times10^{-3}$\\
this paper (Thm.~\ref{thm:upper}) & $0.2538893183$ & $5.144\times10^{-3}$\\
\midrule
improvement & $5.474\times10^{-4}$ & $9.6\%$ of the interval closed\\
\bottomrule
\end{tabular}
\medskip
\caption{The certified upper bound against the previous record and the standing lower
bound. The new bound closes about one tenth of the residual two-sided interval.}
\label{tab:gap}
\end{table}
